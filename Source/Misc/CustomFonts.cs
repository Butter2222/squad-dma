    ...
    public static class CustomFonts
    {
        ...
        static CustomFonts()
        {
            try
            {
                ...
                using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("squad_dma.Resources.NeoSansStdRegular.otf"))
                {
                    fontFamilyRegular = new byte[stream!.Length];
                    stream.ReadExactly(fontFamilyRegular);
                }
                using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("squad_dma.Resources.NeoSansStdBold.otf"))
                {
                    fontFamilyBold = new byte[stream!.Length];
                    stream.ReadExactly(fontFamilyBold);
                }
                using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("squad_dma.Resources.NeoSansStdItalic.otf"))
                {
                    fontFamilyItalic = new byte[stream!.Length];
                    stream.ReadExactly(fontFamilyItalic);
                }
                using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("squad_dma.Resources.NeoSansStdMedium.otf"))
                {

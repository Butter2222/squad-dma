    ...
    public abstract class SKWidget : IDisposable
    {
        ...
        public SKSize Size
        {
            ...
            set
            {
                lock (_sync)
                {
                    if (!float.IsNormal(value.Width) && value.Width != 0f || !float.IsNormal(value.Height) && value.Height != 0f)
                        return;
        ...
        private void InitializeResizeTriangle()
        {
            ...
            path.Close();
            var old = System.Threading.Interlocked.Exchange(ref _resizeTriangle, path);
            old?.Dispose();
        }
        ...
        {
            Color = SKColors.Black.WithAlpha(0x99), // 60% Opacity
            StrokeWidth = 1,
